...
void BufferManager::check_remove_insert(uint64_t pageId){
...
SegmentPage BufferManager::extractId(uint64_t pageId){
	...
	segmentPage.segmentId = pageId >> 48;
	segmentPage.pageId = (pageId << 16) >> 16 >> 16;
	return segmentPage;
}

void BufferManager::upgrade(uint64_t pageId){
	replacement.reference(pageId);
}
