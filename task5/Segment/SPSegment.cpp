...
//else the data is redirected to another page
	if(data.data != nullptr){
		...
		bufferManager->unfixPage(frame, false);
		return std::move(Record(length, data.data));
	}else{
		bufferManager->unfixPage(frame, false);
		...
		return std::move(Record(relength, redata.data));
	}
}
...
uint64_t SPSegment::getSlotCount(uint64_t pageId){
	BufferFrame &frame = bufferManager->fixPage(STID(pageId), false);
	SlottedPage *slottedPage = reinterpret_cast<SlottedPage*>(frame.getData());
	bufferManager->unfixPage(frame, false);
	return slottedPage->getSlotCount();
}
